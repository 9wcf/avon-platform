...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // ✅ الحل الجذري هنا: تغليف الشريط بـ SafeArea لمنع اختفائه تحت شريط الهاتف الأسود
            ...
            child: BackdropFilter(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: const Color(0xFFB76E79).withOpacity(0.25),
                      blurRadius: 25,
  ...
  Widget _navItem(IconData icon, int index, String label) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: selected
              ? const LinearGradient(colors: [Color(0xFFB76E79), Color(0xFFD4909C)])
              : null,
        ...
        child: Row(
          ...
          children: [
            Icon(
              icon,
              size: 22,
              color: selected ? Colors.white : const Color(0xFFB76E79).withOpacity(0.6),
            ),
            if (selected) ...[
...
class _HomeContentState extends State<HomeContent> {
  ...
  Future<void> _loadNotifs() async {
    ...
    if (u == null) return;
    try {
      final all = await Supabase.instance.client.from('notifications').select('*').eq('user_id', u.id).order('created_at', ascending: false).limit(5);
      final unread = await Supabase.instance.client.from('notifications').select('id').eq('user_id', u.id).eq('is_read', false);
      if (mounted) {
        setState(() {
          _notifs = List<Map<String, dynamic>>.from(all);
          _unread = unread.length;
        });
      }
    } catch (_) {}
  }
  ...
  void _subscribeNotif() {
    ...
    _notifChannel = Supabase.instance.client
        ...
        .onPostgresChanges(event: PostgresChangeEvent.insert, schema: 'public', table: 'notifications', filter: PostgresChangeFilter(type: PostgresChangeFilterType.eq, column: 'user_id', value: u.id), callback: (_) => _loadNotifs())
        .onPostgresChanges(event: PostgresChangeEvent.update, schema: 'public', table: 'notifications', filter: PostgresChangeFilter(type: PostgresChangeFilterType.eq, column: 'user_id', value: u.id), callback: (_) => _loadNotifs())
        .subscribe();
  }

  Future<void> _showNotifDropdown() async {
    ...
    await showMenu<Map<String, dynamic>?>(
      context: context,
      position: RelativeRect.fromLTRB(offset.dx + 20, offset.dy + MediaQuery.of(context).padding.top + 70, offset.dx + 40, offset.dy + 100),
      constraints: const BoxConstraints(maxWidth: 340, minHeight: 200),
      ...
      items: [
        const PopupMenuItem<Map<String, dynamic>?>(
          ...
          height: 48,
          child: Row(children: [
            Icon(Icons.notifications, color: Color(0xFFB76E79), size: 22),
            SizedBox(width: 8),
            Text('الإشعارات', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Color(0xFF333333), fontFamily: 'Tajawal')),
          ]),
        ),
        ...
        if (_notifs.isEmpty)
          const PopupMenuItem<Map<String, dynamic>?>(
            ...
            child: Center(
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.notifications_none, size: 36, color: Color(0xFFE8B4B8)),
                SizedBox(height: 8),
                Text('لا توجد إشعارات', style: TextStyle(fontSize: 14, fontFamily: 'Tajawal', color: Color(0xFF999999))),
              ]),
            ),
          )
        else
          ..._notifs.map((n) {
            ...
            Color color = const Color(0xFFB76E79);
            IconData icon = Icons.notifications;
            if (type == 'approved') { color = const Color(0xFF2E7D32); icon = Icons.check_circle; }
            else if (type == 'rejected') { color = const Color(0xFFC62828); icon = Icons.error; }
            else if (type == 'admin_message') { color = const Color(0xFF1976D2); icon = Icons.chat; }
            else if (type == 'new_booking') { color = const Color(0xFFD4AF37); icon = Icons.event; }

            final created = (n['created_at'] ?? '').toString();
            final time = created.length >= 16 ? created.replaceAll('T', ' ').substring(11, 16) : '';
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text((n['title'] ?? '').toString(), style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: const Color(0xFF333333), fontFamily: 'Tajawal'), maxLines: 1, overflow: TextOverflow.ellipsis),
                          const SizedBox(height: 2),
                          Text((n['body'] ?? '').toString(), style: TextStyle(fontSize: 11, color: const Color(0xFF666666), fontFamily: 'Tajawal'), maxLines: 2, overflow: TextOverflow.ellipsis),
                          if (time.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 2), child: Text(time, style: TextStyle(fontSize: 9, color: Colors.grey.shade500, fontFamily: 'Tajawal'))),
                        ],
                      ),
                    ),
                    if (!isRead) Container(width: 8, height: 8, margin: const EdgeInsets.only(top: 8, right: 4), decoration: const BoxDecoration(color: Color(0xFFC62828), shape: BoxShape.circle)),
                  ],
                ),
              ),
            );
          ...
          const PopupMenuItem<Map<String, dynamic>?>(
            ...
            child: Center(
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.list, size: 16, color: Color(0xFFB76E79)),
                SizedBox(width: 6),
                Text('عرض كل الإشعارات', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFFB76E79), fontFamily: 'Tajawal')),
              ]),
            ),
          ),
      ],
      ...
      if (value != null) {
        await Supabase.instance.client.from('notifications').update({'is_read': true}).eq('id', value['id']);
        _loadNotifs();
      }
    });
  }
  ...
  Widget _img(String src) {
    if (src.startsWith('http')) return Image.network(src, fit: BoxFit.cover, errorBuilder: (c, e, s) => _placeholder());
    return Image.asset(src, fit: BoxFit.cover, errorBuilder: (c, e, s) => _placeholder());
  }

  Widget _placeholder() {
    return Container(decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFE8B4B8), Color(0xFFB76E79)])), child: const Center(child: Icon(Icons.spa, color: Colors.white, size: 40)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        // ✅ مسافة من الأسفل لضمان ظهور الفروع بشكل كامل عند التمرير
  ...
  Widget _buildOffersBanner() {
    if (_offers.isNotEmpty) {
      return SizedBox(
        height: 180,
        child: ListView.builder(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 20), itemCount: _offers.length, itemBuilder: (context, index) => _offerCard(_offers[index])),
      );
    }
    return Container(
      height: 180,
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: Colors.white.withOpacity(0.3), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white.withOpacity(0.5))), child: Text((_settings['hero_badge'] ?? '✨ عرض خاص').toString(), style: const TextStyle(color: Colors.white, fontSize: 11, fontFamily: 'Tajawal', fontWeight: FontWeight.w600))),
                  const SizedBox(height: 10),
                  Flexible(child: Text((_settings['hero_title'] ?? 'جمالك يستحق الأفضل').toString(), style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w300, color: Colors.white, fontFamily: 'Tajawal', height: 1.1), maxLines: 2, overflow: TextOverflow.ellipsis)),
                  const SizedBox(height: 14),
                  GestureDetector(
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const BookingScreen(serviceName: 'تجميل البشرة'))),
                    child: Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20)), child: const Row(mainAxisSize: MainAxisSize.min, children: [Text('احجزي الآن', style: TextStyle(color: Color(0xFFB76E79), fontSize: 13, fontWeight: FontWeight.w700, fontFamily: 'Tajawal')), SizedBox(width: 6), Icon(Icons.arrow_back, color: Color(0xFFB76E79), size: 16)])),
                  ),
                ],
              ),
            ),
  ...
  Widget _offerCard(Map<String, dynamic> offer) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context, 
        MaterialPageRoute(builder: (_) => BookingScreen(serviceName: (offer['title_ar'] ?? '').toString()))
      ),
      child: Container(
        width: 280,
              ...
              Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [Colors.transparent, const Color(0xFF7A3B47).withOpacity(0.9)]
                  )
                )
              ),
              Positioned(
                ...
                child: Container(
                  ...
                  child: Text('خصم $discount%', style: const TextStyle(color: Color(0xFFB76E79), fontSize: 12, fontWeight: FontWeight.w800, fontFamily: 'Tajawal')),
                ),
              ),
              Positioned(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Flexible(
                          child: Text('${offer['discounted_price']} د.ع', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Colors.white, fontFamily: 'Tajawal'), overflow: TextOverflow.ellipsis)
                        ),
                        const SizedBox(width: 10),
                        Text('${offer['original_price']} د.ع', style: TextStyle(fontSize: 12, color: Colors.white.withOpacity(0.7), decoration: TextDecoration.lineThrough, fontFamily: 'Tajawal')),
                      ],
                    ),
                  ],
                ),
  ...
  Widget _buildServices() {
    if (_services.isEmpty) {
      return const SizedBox(
        height: 60,
        child: Center(child: Text('لا توجد خدمات', style: TextStyle(fontFamily: 'Tajawal'))),
      );
    }
    return SizedBox(
      height: 220,
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final name = (s['name_ar'] ?? '').toString();
          final price = (s['price_iqd'] ?? 0).toString();
          final duration = (s['duration_minutes'] ?? 45).toString();
          return GestureDetector(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Text('$duration دقيقة', style: TextStyle(fontSize: 11, color: const Color(0xFF333333).withOpacity(0.6), fontFamily: 'Tajawal'), overflow: TextOverflow.ellipsis),
                          ],
                        ),
                      ],
                    ),
    ...
    if (_offers.isEmpty) {
      return Padding(
        ...
        child: Container(
          ...
          child: const Row(children: [Icon(Icons.local_offer, color: Color(0xFFB76E79)), SizedBox(width: 12), Expanded(child: Text('أضيفي عروضاً من لوحة التحكم', style: TextStyle(fontSize: 13, fontFamily: 'Tajawal', color: Color(0xFF333333)), overflow: TextOverflow.ellipsis))]),
        ),
      );
    }
    return SizedBox(
                  ...
                  Stack(
                    children: [
                      Container(
                        ...
                        child: imgUrl.startsWith('http') ? ClipOval(child: Image.network(imgUrl, fit: BoxFit.cover, errorBuilder: (c, e, s) => const Icon(Icons.person, color: Colors.white, size: 30))) : const Icon(Icons.person, color: Colors.white, size: 30),
                      ),
          ...
          return Container(
            ...
            child: Row(children: [
              ...
              SizedBox(width: 150, child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                Text((b['name_ar'] ?? '').toString(), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: Color(0xFF333333), fontFamily: 'Tajawal'), maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 3),
                Text((b['address'] ?? '').toString(), style: TextStyle(fontSize: 10, color: const Color(0xFF333333).withOpacity(0.5), fontFamily: 'Tajawal'), maxLines: 1, overflow: TextOverflow.ellipsis),
              ])),
            ]),
          ).animate().fadeIn(delay: Duration(milliseconds: index * 80), duration: 500.ms);
        },
